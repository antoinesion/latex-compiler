...
\\documentclass[%(main_package)s,border={%(padding_left).1fpt %(padding_bottom).1fpt %(padding_right).1fpt %(padding_top).1fpt},multi=false]{standalone}
    ...
    with sentry_sdk.start_transaction(op="task", name="to-jpg"):
        ...
        try:
            ...
            if not width is None:
                ...
                main_package = b'preview'
            else:
                width_constrain = b''
                main_package = b'crop'

            os.write(input_file, LATEX_TEMPLATE % {
                b'main_package': main_package,
